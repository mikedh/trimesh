...
def light(scene, warm=(255, 244, 224, 255), cool=(198, 216, 255, 255)):
    ...
    # would simply cast a shadow over everything. the shell lights
    # itself, so these only have to light the subject
    rig = [
...
def lightbox(scene, radius=18.0, width=0.25, dim=0.10, name="lightbox"):
    """
    Put the subject inside a self-lit backdrop.

    A lightbox is a physical object, so building it as geometry means the
    file carries its own studio and every renderer sees the same thing.
    It is also what the polished parts reflect, which is most of what
    makes them look polished.

    The shell is emissive rather than lit. That is not a stylistic
    choice: a lit backdrop is darkened wherever the subject blocks the
    lights, and no amount of moving them removes that shadow, only
    resizes it. Emission ignores occlusion, so a self-lit shell cannot
    be shadowed at all, and it frees the light rig to worry only about
    the subject.

    Call this after `orbit` and `light`, which both frame themselves
    against the scene bounds and would otherwise fit to the shell.
    """
    ...
    shell = trimesh.creation.icosphere(subdivisions=4, radius=outer)
    # we are looking at this from the inside, and GLTF culls back faces
    # by default, so the normals have to point inwards
    shell.invert()

    # run the gradient up the camera's own up axis rather than the drive
    # axis: the camera only ever sees a small patch of a shell this size,
    # and that patch spans three times as much of the camera axis as it
    # does of any other, which is the difference between a sweep you can
    # see and a flat wall
    # a camera's up axis is perpendicular to where it points, so a
    # gradient about the shell's center already lands centered in frame
    up = scene.graph[scene.camera.name][0][:3, 1]
    # `tanh` rather than a clipped ramp: clipping happens per vertex, so
    # its boundary follows the triangle edges and renders as a sawtooth
    # across the backdrop. this saturates smoothly and never reaches
    # either end, so there is no boundary to see
    blend = 0.5 + 0.5 * np.tanh((shell.vertices @ up) / (outer * width))

    # a one dimensional ramp, sampled by V. note GLTF puts the V origin
    # at the top of the image, so this is flipped to put light up there
    ramp = np.linspace(dim, 1.0, 256).reshape((-1, 1, 1)) * [[[1.0, 1.005, 1.03]]]
    shell.visual = trimesh.visual.TextureVisuals(
        uv=np.column_stack([np.full(len(blend), 0.5), 1.0 - blend]),
        material=trimesh.visual.material.PBRMaterial(
            name=name,
            # black and unlit, so nothing can cast a shadow onto it
            baseColorFactor=np.array([0.0, 0.0, 0.0, 1.0]),
            metallicFactor=0.0,
            roughnessFactor=1.0,
            emissiveFactor=np.array([1.0, 1.0, 1.0]),
            emissiveTexture=Image.fromarray(
                (np.clip(ramp, 0.0, 1.0) * 255).astype(np.uint8).repeat(8, axis=1), "RGB"
            ),
        ),
    )
...
if __name__ == "__main__":
    ...
    # `flatten=False` keeps the crank axes this animation is keyed to
    scene = scene.convert_units("m", flatten=False)
