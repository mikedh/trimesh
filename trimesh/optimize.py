import numpy as np
import trimesh, time, fem, alm, to, meshers, picker
from multiprocessing import Process

class topology(object):

    def __init__(self,Mesh,grid_size,E,nu,vol_fraction,radius,forces,clamp=True,
                preview=True,penal=3.,Flag_AM=False,scene_update=5,max_iter=150,IterativeSolver=False):
        """
        
        Parameters
        -------------
        Mesh: (trimesh.array) Surface mesh of the design domain
        grid_size: (float) Number of voxel elements in largest dimension (X or Y or Z)
        ...
        Returns
        ----------
        self.NewMesh: (trimesh.array) new surface mesh of the optimized design volume
        self.Convergence: (np.array(it,5)) convergence history: 
        ...
        # Call optimization
        opt=optimize.topology(mesh,grid_size,E,nu,vol_fraction,radius,forces,Flag_AM,max_iter=50)  
        mesh2=opt.NewMesh
        ...
        # Show smoothen optimized geometry
        ...
        for i in range(1,max_iter):         
                ...
                # Update Mesh scene  every 5 iterations after iteration 14
        ...
        self.NewMesh = meshers.skin(HexNodes,HexElements,xprint,vol_fraction)
        self.Convergence=np.asarray(convergence)