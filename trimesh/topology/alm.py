...
def support_region_search(i, Elements, Centroid, Neighbors, v_build):
    ...
    First entry is element below for multiplier
    Parameters
    -------------
    i: (int) element index
    ...
    others = []
    central = []
    # Searches for the supporting region within neighbor region
        ...
        if i != j:
            ...
            if flag:
                ...
                if cos_angle < -0.98:
                    ...
                    others.extend([j])
    # Making sure the first entry is the element in the layer below
...
def print_elm_sorted(Centroid, v_build):
    ...
    """
    fem_print_order = [
        print_elm_weights(p, Centroid, v_build) for p in range(len(Centroid))
    ]
...
def region_search(Elements, Centroid, Neighbors):
    ...
    # print direction
    v_build = np.array([0.0, 1.0, 0.0])

    # Compute Aux regions
    support_region = [
        support_region_search(i, Elements, Centroid, Neighbors, v_build)
        for i in range(len(Centroid))
    ]
    reverse_region = [
        reverse_support_region_search(i, support_region) for i in range(len(Centroid))
    ]
    ...
    if len(dc) > 0:
        ...
        for i in reversed(fem_print_order):
            ...
            if ns > 1:
                ...
                # j Elements in the same layer using elements that share the same
                # blueprint
        ...
        for i in reversed(fem_print_order):
            ...
            if ns > 1:
                ...
                # Update Lagrange Multiplier Below
                k = 0
                for j in reverse_region[u]:
                    lamb_dc[u] += (
                        (dc_old[j] + lamb_dc[j]) * dSmindSmax[j] * dSmaxdx[i][k]
                    )
                    lamb_dv[u] += (
                        (dv_old[j] + lamb_dv[j]) * dSmindSmax[j] * dSmaxdx[i][k]
                    )
                    k += 1
            else:
                # Filtering dc dv
