...
def fem_solve(E,nu,penal,xprint,Nodes,Elements,bc_u,bc_f,forces,Iterative_Solver):
    """
    Solves the FEM problem considering a regular hex8 (cube) voxel mesh. For futher
    ...
    "binbox" for arbitary designs volumes and precomputed sitiffness matrices.
    ...
    # Get dofs
    consdofs=np.concatenate((np.asarray(bc_u[0],dtype=int)*3,np.asarray(bc_u[1],
                                dtype=int)*3+1,np.asarray(bc_u[2],dtype=int)*3+2))
    ...
    if Iterative_Solver==False:
        U[freedofs,0]=spsolve(KG,F[freedofs,0])
    else:
        U[freedofs,0]=bicgstab(KG,F[freedofs,0])[0]

    #print("\n Computes Strain Energy")
    ese= (aux_xprint**penal*E)*((np.dot(U[edofmat].reshape(-1,24),KE) * U[edofmat].reshape(-1,24) ).sum(1))
