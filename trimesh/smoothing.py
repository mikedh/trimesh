...
def filter_mut_dif_laplacian(mesh,
  ...
  vertices = mesh.vertices.copy().view(np.ndarray)
  faces    = mesh.faces.copy().view(np.ndarray)
  #neigh    = mesh.vertex_neighbors.copy().view(np.ndarray)
  #nearby_faces = trimesh.proximity.nearby_faces(mesh,mesh.vertices)
  #centers=mesh.triangles_center.copy().view(np.ndarray)
  ...
  for _index in range(iterations):
    ...
    pi_qi = vertices-qi
    adil = (normals*pi_qi).dot(np.ones((3,1)))

    vertices -= lamb*normals*adil
  ...
  # Set Volume Residual to zero
  if volume_constraint==True:
        vol2=mass_properties(vertices[faces],skip_inertia=True)["volume"]
        vertices *= ((v_ini/vol2)**(1./3.))             
