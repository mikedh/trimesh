...
def xaml_load(file_obj, *args, **kwargs):
    ...
    for geometry in root.iter(tag=ns+'GeometryModel3D'):
        ...
        diffuse = geometry.find('.//{}DiffuseMaterial/*/{}SolidColorBrush'.format(ns,ns))
        specular = geometry.find('.//{}SpecularMaterial/*/{}SolidColorBrush'.format(ns,ns))
        ...
        while current is not None:            
            ...
            if transform_element is not None:
                # we are traversing the tree backwards, so append new transforms left
