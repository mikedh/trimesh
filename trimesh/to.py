...
def laplacian_calculation(Centroid, Neighbors, radius):
    ...
    ones = np.ones(3)
    norms = [ (radius-np.sqrt(np.dot((Centroid[i] - Centroid[n]) ** 2, ones)))
...
def to_grad_calc(n,xprint,ese,penal,vol_fraction):
    ...
    penal: (float) Penalty constant of Topology Optmization (from 2 to 6)
...
def to_grad_filt(n,xprint,xphy,xfilt,mfilter,
                 support_region,reverse_region,fem_print_order,
                 ese,dc,dv,penal,beta,eta,fta):
    """
    ...
    if fta == True:
        xprint,dc,dv = alm.ALMfilter(support_region,reverse_region,fem_print_order,xphy,dc,dv)

    # Partial Derivative of Heavisidade Projection
    dx=beta*(1.-(np.tanh(beta*(xfilt-eta)))**2.)/(np.tanh(beta*eta)+np.tanh(beta*(1.-eta)))
    dc_aux  = dc*dx
    dv_aux  = dv*dx
...
def filt_den(vol_fraction,xval,mfilter,support_region,reverse_region,fem_print_order,beta,eta,fta):
    """
    Filtering densities maps in order to avoid checker board phenomena and impose minimum member size
    Parameters
    -------------
    vol_fraction: (float) Volume Fraction Constraint
    ...
    #Projection
    xphy=(np.tanh(beta*eta)+np.tanh(beta*(xfilt-eta)))/(np.tanh(beta*eta)+np.tanh(beta*(1.0-eta)))
    ...
    Mn=4.*np.abs(xprint*(1-xprint)).sum()/len(xprint)
