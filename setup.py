...
_version_file = os.path.join(os.path.dirname(__file__), "trimesh", "version.py")
...
)  # used as a validator for exports

# things that are used implicitly
requirements_recommends = set(["meshio", "sympy", "glooey"])
...
if "--list-all" in sys.argv:
    ...
    print("version bump `{}` => `{}`".format(__version__, version_new))
