...
class CurvatureTest(g.unittest.TestCase):
    ...
    def test_mean_curvature(self):
        ...
        mean = g.trimesh.curvature.discrete_mean_curvature_measure(
            m,
            m.vertices,
            2.0) / (4 * g.np.pi)
        # Check mean curvature for unit sphere is 1.0
        assert g.np.allclose(mean, 1.0, atol=.01)
    ...
    def test_vertex_defect(self):
        ...
        assert g.np.logical_or(g.np.isclose(m.vertex_defects, 0.0),
                               g.np.isclose(m.vertex_defects, g.np.pi / 2.0)).all()
