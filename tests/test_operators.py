...
class OpTest(g.unittest.TestCase):
    ...
    def test_add(self):
        ...
        m = [g.trimesh.creation.box().apply_translation(v)
             for v in 2.0 * g.np.eye(3)]
        assert g.np.isclose(sum(m).volume, 3.0)
        assert g.np.isclose(g.np.sum(m).volume, 3.0)
        assert g.np.isclose((m[0] + m[1] + m[2]).volume, 3.0)
        assert g.np.isclose(
            g.trimesh.util.concatenate(m).volume, 3.0)

        p = g.get_mesh('2D/wrench.dxf')
        m = [p.copy().apply_translation(v)
             for v in p.extents.max() * 2.0 * g.np.eye(2)]
        m.append(p)
        ...
        assert g.np.isclose(
            g.trimesh.path.util.concatenate(m).area, area)
